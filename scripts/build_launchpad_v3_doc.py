...
def build():
    ...
    add_para(
        doc,
        "创建Token / 30个0（30个零）供应量 / 默认BNB底池 / 自定义手续费 / 普通白名单与限购版名单",
        size=12,
        color="555555",
        after=14,
    )
    add_para(
        doc,
        "定位：这是一个偏傻瓜式的多零代币创建工具。项目方填写名称、符号、供应量和手续费配置后，即可创建带默认BNB底池和白名单能力的代币。创建费用为0.005 BNB，后续由创建者钱包完成加池、开盘和运营管理。",
        after=10,
    )
    ...
    label_table(
        ...
        [
            ...
            ("核心卖点", "支持0精度和30个0（30个零）供应量，默认BNB底池，支持买入/卖出手续费自定义、Pancake加池和手动开盘"),
    ...
    add_heading(doc, "三、手续费配置", 1)
    add_para(
        doc,
        "平台创建的每个代币由项目方自行填写手续费配置，买入和卖出可以分别设置流动性、营销、分红和销毁。税收在链上可查，业务上以合约参数和项目方填写内容为准。",
        after=8,
    )
    matrix_table(
        doc,
    ...
    add_bullet(doc, "系统按项目方填写的买入/卖出手续费配置创建代币。")
    ...
    matrix_table(
        ...
        ["页面模块", "建议字段", "说明"],
        [
            ("创建Token", "名称、符号、供应量、默认BNB底池", "保持简单，默认值直接给30个0供应量，手续费由项目方自行填写。"),
            ("税收设置", "营销地址、买入手续费、卖出手续费", "买入和卖出分别填写流动性、营销、分红和销毁。"),
    ...
    add_bullet(doc, "所有通过平台创建的币，手续费由创建者自行填写并写入合约。")
