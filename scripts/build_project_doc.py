...
def build():
    ...
    add_para(
        doc,
        "默认买卖总税按 3% 设计。项目/营销钱包拿的是所有税收的 20%，不是交易额的 20%；换算后为交易额的 0.6%，按直达钱包逻辑进入指定地址。其余税收按合约配置执行，其中销毁 0.5%，回流 0.5%；回流用于加池，产生的 LP 进入黑洞地址，避免后续被取回。链上税收无法真正隐藏，源码和交易记录都能查到；前端展示可以简化。",
        after=8,
    )
    matrix_table(
        doc,
        ["项目", "比例", "说明"],
        [
            ("默认总税", "3%", "买入和卖出默认按3%总税收计算；如需不同税率，可部署前调整。"),
            ("项目/营销钱包", "税收的20% / 交易额0.6%", "不是额外20%税，而是默认3%税收里的20%，交易触发后直达指定钱包。"),
    ...
    add_bullet(doc, "项目/营销钱包：拿默认总税收的20%，默认总税3%时等于交易额0.6%，交易触发后直达指定地址。")
    ...
    add_bullet(doc, "项目/营销钱包拿默认总税收的20%，默认总税3%时实际为交易额0.6%，并做成直达指定钱包。")
    add_bullet(doc, "销毁默认0.5%，回流默认0.5%，回流加池产生的LP进入黑洞地址。")
    add_bullet(doc, "持币分红使用USDT，剩余税收按分红和其他配置执行。")
    add_bullet(doc, "默认总税、分红、销毁、回流、黑洞地址等参数需部署前确认。")
